...
func TestPreflightsCLI(t *testing.T) {
	...
	assert.Equal(t, manager.cluster.CLI.Path, sacctPath)
}
...
func TestParseSacctCmdOutput(t *testing.T) {
	...
	assert.Equal(t, units[0].TotalTime["walltime"], models.JSONFloat(3600))
	...
	// Check if elapsed time corresponds to real elapsed time of job
	assert.Equal(t, units[0].TotalTime["walltime"], models.JSONFloat(0))
	...
	// Check if elapsed time corresponds to real elapsed time of job
	assert.Equal(t, units[0].TotalTime["walltime"], models.JSONFloat(300))
	...
	// Check if elapsed time corresponds to real elapsed time of job
	assert.Equal(t, units[0].TotalTime["walltime"], models.JSONFloat(600))
	...
	// Check if elapsed time corresponds to real elapsed time of job
	assert.Equal(t, units[0].TotalTime["walltime"], models.JSONFloat(120))
}
...
func TestParseSacctMgrCmdOutput(t *testing.T) {
	...
	require.ElementsMatch(t, users, expectedUsers)
	require.ElementsMatch(t, projects, expectedProjects)
}
