...
avg_over_time(
	avg by (batchjobid) (
		(
			rate(batchjob_slurm_job_cpu_user_seconds{batchjobid=~"%[1]s"}[%[2]s])
			+
			rate(batchjob_slurm_job_cpu_system_seconds{batchjobid=~"%[1]s"}[%[2]s])
		)
		/
		batchjob_slurm_job_cpus{batchjobid=~"%[1]s"}
	)[%[3]s:]
) * 100`
...
avg_over_time(
	avg by (batchjobid) (
		batchjob_slurm_job_memory_used_bytes{batchjobid=~"%[1]s"}
		/
		batchjob_slurm_job_memory_total_bytes{batchjobid=~"%[1]s"}
	)[%[3]s:]
) * 100`
...
avg_over_time(
	avg by (batchjobid) (
		...
		* on (UUID) group_right ()
		label_replace(
			batchjob_slurm_job_gpu_index_flag{batchjobid=~"%[1]s"},
			"UUID",
			"$1",
			"uuid",
			"(.*)"
