...
func TestTSDBConfigSuccess(t *testing.T) {
	...
	// Check if Ping is working
	if err := tsdb.Config(); err != nil {
		t.Errorf("Could not get TSDB config")
	}
	if v, exists := tsdb.globalConfig["scrape_interval"]; !exists {
	...
	// Set rate interval
	tsdb.RateInterval()
	if tsdb.rateInterval != time.Duration(60*time.Second) {
		t.Errorf("expected rate_interval 60s got %s", tsdb.rateInterval)
	}
}
...
func TestTSDBConfigFail(t *testing.T) {
	...
	tsdb.RateInterval()
	if tsdb.rateInterval != time.Duration(defaultScrapeInterval)*4 {
		t.Errorf("Expected default rate interval, got %s", tsdb.rateInterval)
	}
}
...
func TestTSDBQuerySuccess(t *testing.T) {
	...
	if m, err := tsdb.Query("", time.Now()); err != nil {
		...
		if !reflect.DeepEqual(m, map[int64]float64{1: 1.1, 2: 2.2}) {
