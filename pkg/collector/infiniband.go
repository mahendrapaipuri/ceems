...
func init() {
	RegisterCollector(ibSubsystem, defaultDisabled, NewInfiniBandCollector)
}
...
func NewInfiniBandCollector(logger *slog.Logger) (Collector, error) {
	...
	i.metricDesc = make(map[string]*prometheus.Desc)

	for metricName, description := range descriptions {
		i.metricDesc[metricName] = prometheus.NewDesc(
			prometheus.BuildFQName(Namespace, ibSubsystem, metricName),
			description,
			[]string{"device", "port"},
			nil,
		)
...
// Update updates metric channel with IB stats.
func (c *infinibandCollector) Update(ch chan<- prometheus.Metric) error {
	...
	for _, device := range devices {
		infoDesc := prometheus.NewDesc(
			prometheus.BuildFQName(Namespace, ibSubsystem, "info"),
...
func (c *infinibandCollector) Stop(_ context.Context) error {
	c.logger.Debug("Stopping", "collector", ibSubsystem)
...
func (c *infinibandCollector) pushMetric(ch chan<- prometheus.Metric, name string, value uint64, deviceName string, port string, valueType prometheus.ValueType) {
	ch <- prometheus.MustNewConstMetric(c.metricDesc[name], valueType, float64(value), deviceName, port)
}
